...
class ZImagePipeline(BasePipeline):
    ...
    def __call__(
        ...
        if inputs_shared.get("latents") is None:
            if inputs_shared.get("noise") is None:
                inputs_shared["noise"] = self.generate_noise((1, 16, height//8, width//8), seed=seed, rand_device=rand_device, rand_torch_dtype=self.torch_dtype)
...
def model_fn_z_image(
    ...
    **kwargs,
    ...
    if latents is None:
        latents = kwargs.get("latents") or kwargs.get("noise") or kwargs.get("input_latents")
    if latents is None:
        height = kwargs.get("height", 512)
        width = kwargs.get("width", 512)
        device = next(dit.parameters()).device
        dtype = next(dit.parameters()).dtype
        latents = torch.randn((1, 16, height // 8, width // 8), device=device, dtype=dtype)
